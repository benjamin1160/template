...
export default function BigNumbers({ bigNumbers }: BigNumbersProps) {
  ...
  return (
    <section className="border-y border-border bg-bg py-16 sm:py-20 md:py-24">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-8 md:gap-12">
          {stats.map((stat, i) => (
            <div key={i} className="text-center">
              <div className="display text-6xl md:text-7xl lg:text-8xl text-accent tabular-nums">
                {stat.value}
              </div>
              <div className="mt-3 text-sm md:text-base font-bold uppercase tracking-wider text-fg">
                {stat.label}
              </div>
              {stat.sublabel && (
